...
public class ConnectionAcceptor<T> {
	...
	public void accept() throws IOException {
		...
		if (sChannel != null) {
			...
			ConnectionHandler_Reactor<T> handler = ConnectionHandler_Reactor.create(sChannel,_data, key);		
			handler.sendLine("Welcome to the SPL Internet Relay Chat Network. Your host is "+ sChannel.getLocalAddress()+ " running version 1.0 ");
			// in order for the client to recive the welcome message immediately  we switched to writeonlymode.
			handler.switchToWriteOnlyMode();
