 ...
 */
public class ConnectionHandler_Reactor<T> extends ConnectionHandler<T>{
	 ...
	 */
	private ConnectionHandler_Reactor(SocketChannel sChannel,Encoder encoder, Serializer<T> serializer, MessagingProtocol<T> protocol,
			ReactorData<T> data, SelectionKey key) {
		super(encoder, serializer, protocol);
		_sChannel = sChannel;
		_data = data;		
		_protocol.bind((ConnectionHandler<T>) this);
		_skey = key;
	}
	...
	private void initialize() {
		_skey.attach(this);
		_task = new ProtocolTask<T>(_protocol, (MessageSerializer<T>) _serializer);
	}

	public static <T> ConnectionHandler_Reactor<T> create(SocketChannel sChannel , ReactorData<T> data, SelectionKey key) {
		Serializer<T> serializer = data.getTokenizerMaker().create();
		MessagingProtocol<T> protocol = (MessagingProtocol<T>) data.getProtocolMaker().create();
		Encoder encoder = data.getEncoder();

		ConnectionHandler_Reactor<T> h = new ConnectionHandler_Reactor<T>(sChannel , encoder , serializer, protocol ,data, key);
		h.initialize();
		return h;
	}
...
/** this method receive arralylist of byte array that the protocol 
 * sent as response to the client, and send it to the 
 * original addOutData method of the course implementation.
 * 
 ...
 */
	...
	public synchronized void sendLines (ArrayList<byte[]> lines){		
		for (int i=0;i<lines.size();i++){	
			ByteBuffer buffer = ByteBuffer.wrap(lines.get(i));
